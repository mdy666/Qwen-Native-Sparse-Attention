...
def _fused_apply_rope_bwd(  DQ_EMBED, 
    ...
    DQ += start_n * QH * D
    DQ_EMBED += start_n * QH * D
    DK += start_n * KH * D
    DK_EMBED += start_n * KH * D
    COS += off_b * (COS_B // B) * L * D + off_l * D
    SIN += off_b * (COS_B // B) * L * D + off_l * D
    ...
    dk_embed1 = tl.load(DK_EMBED + tl.arange(0, BLOCK_KH)[:, None] * D + cols[None, :], 
                 mask=(cols[None, :]<HALF_D) & (tl.arange(0, BLOCK_KH)[:, None] < KH), other=0.).to(tl.float32)
    dk_embed2 = tl.load(DK_EMBED + tl.arange(0, BLOCK_KH)[:, None] * D + cols[None, :] + HALF_D, 
                 mask=((HALF_D+cols[None, :])<D) & (tl.arange(0, BLOCK_KH)[:, None] < KH), other=0.).to(tl.float32)
    dk1 = dk_embed1 * cos1 + sin2 * dk_embed2
    dk2 = dk_embed2 * cos2 - sin1 * dk_embed1
    tl.store(DK + tl.arange(0, BLOCK_KH)[:, None] * D + cols[None, :], dk1, mask=(cols[None, :]<HALF_D)&(tl.arange(0, BLOCK_KH)[:, None] < KH))
    tl.store(DK + tl.arange(0, BLOCK_KH)[:, None] * D + cols[None, :] + HALF_D, dk2, mask=((HALF_D+cols[None, :])<D) & (tl.arange(0, BLOCK_KH)[:, None] < KH))

    dq_embed1 = tl.load(DQ_EMBED + tl.arange(0, BLOCK_QH)[:, None] * D + cols[None, :], 
                 mask=(cols[None, :]<HALF_D) & (tl.arange(0, BLOCK_QH)[:, None] < QH), other=0.).to(tl.float32)
    dq_embed2 = tl.load(DQ_EMBED + tl.arange(0, BLOCK_QH)[:, None] * D + cols[None, :] + HALF_D, 
                 mask=((HALF_D+cols[None, :])<D) & (tl.arange(0, BLOCK_QH)[:, None] < QH), other=0.).to(tl.float32)
    dq1 = dq_embed1 * cos1 + sin2 * dq_embed2
    dq2 = dq_embed2 * cos2 - sin1 * dq_embed1
    tl.store(DQ + tl.arange(0, BLOCK_QH)[:, None] * D + cols[None, :], dq1, mask=(cols[None, :]<HALF_D)&(tl.arange(0, BLOCK_QH)[:, None] < QH))
    tl.store(DQ + tl.arange(0, BLOCK_QH)[:, None] * D + cols[None, :] + HALF_D, dq2, mask=((HALF_D+cols[None, :])<D) & (tl.arange(0, BLOCK_QH)[:, None] < QH))
